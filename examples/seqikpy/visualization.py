...
def animate_3d_points(
    ...
    points3d : Dict[str, np.ndarray]
        ...
        usually this is the raw 3D pose.(solid line)
    export_path : Path
        Path where the animation will be saved.
    points3d_second : Dict[str, np.ndarray], optional
        Dictionary containing the 3D pose
        usually this is the forward kinematics, by default None (dotted line)
    fps : int, optional
...
def fig_to_array(fig):
    ...
    data = data[:, :, :3] # RGBA formatını RGB'ye dönüştürmek için
    return data
