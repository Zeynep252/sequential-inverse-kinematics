...
with left_space:
    ...
    if selection == "Locomotion - Forward Kinematics Sample":
        ...
        options=[
            "Aligned Pose Data",
            "Forward Kinematics",
            "Aligned Pose + Forward Kinematics",
            "Leg Joint Angles (Run Forward Kinematics)"
        ])
        ...
        if (pose_type_selection=="Aligned Pose Data") or (pose_type_selection=="Forward Kinematics"):
            ...
            if uploaded_file is not None:
                ...
                if pose_type_selection=="Aligned Pose Data":
                    st.markdown("**Would you like to run inverse and forward kinematics on your data?**")
                    if st.button("Run Kinematics"):
                        ...
                        def streamlit_trange(
                            ...
                            **kwargs,
                            ...
                            progress_bar = progress_area.progress(
                                0,
                                text=desc,
                            )
                        ...
                        st.markdown("**Visualizing forward kinematics output with aligned pose**\n\n____ Solid lines: Aligned pose\n\n------- Dashed lines: SeqIKPy Forward Kinematics")
                        fig.data=[]
                        try:
                            visualize(poseData3D,FKData)
                        except Exception as e:
                            st.error(f"❌ Cannot visualize data: {e}")
                        ...
                        with zipfile.ZipFile(
                            ...
                            zip_file.writestr(
                                f"{uploaded_file.name}_forward_kinematics.pkl",
                                pkl_FK_data,
                            )

                            zip_file.writestr(
                                f"{uploaded_file.name}_leg_joint_angles.pkl",
                                pkl_joint_angles_data,
                            )
                        st.download_button(
                            label="Download Joint Angles & Forward Kinematics Data as ZIP",
                            data=zip_buffer.getvalue(),
                            file_name=f"{uploaded_file.name}-Kinematics-Output.zip",
                            mime="application/zip",
                            on_click="ignore",
                            key="download_kinematics_results",
                        )

        elif pose_type_selection=="Aligned Pose + Forward Kinematics":
        ...
        elif pose_type_selection=="Leg Joint Angles (Run Forward Kinematics)":
            uploaded_file= st.file_uploader(
            ...
            if (uploaded_file is not None):
                try:
                    ...
                    with process_space:
                        AngleData = pickle.load(uploaded_file)
                        st.info(f"Running Forward kinematics on your data")
                    ...
                    st.download_button(
                        label="Download FK data as .pkl",
                        data=pkl_FK_data,
                        file_name=f"{uploaded_file.name}- Forward Kinematics.pkl",
                    ...
                    st.error(f"❌ Cannot process/visualize data: {e}")
